...
See the file script for an example of the file format
"""
ARG_COMMANDS = [ 'box', 'sphere', 'torus', 'circle', 'bezier', 'vertex', 'hermite', 'line', 'scale', 'move', 'rotate', 'save' ]
...
def parse_file( fname, edges, polygons, transform, screen, color ):
    ...
    while c < len(lines):
        ...
        if line == 'vertex':
            add_point(polygons, 100*float(args[0]), 100*float(args[1]), 100*float(args[2]))
